...
__device__ float* vectorDir(int);
__device__ int closestDir(float*);


//Constants
const char *FILE_NAME = "ducks-16.raw";
const int MAX_NAME = 512;               // max length of string
const int IMAGE_X = 2400;                // image width
const int IMAGE_Y = 2400;                // image height
const int APRON = 3;
...
__global__ void filterVector(int* dataMatrix, int* vectorMatrix, int X, int Y) {
	int i = (blockIdx.x * blockDim.x) + threadIdx.x;
	int j = (blockIdx.y * blockDim.y) + threadIdx.y;
	int index = j * X + i;
...
__device__ int findVectorSum(int* matrix, int index, int X) {
	float sum[2] = { 0 };
	for (int i = 1; i < 4; i++) {
		float * vec1 = vectorDir(matrix[index - (X*i)]);
		float * vec2 = vectorDir(matrix[index + (X*i)]);
		sum[0] = sum[0] + vec1[0] + vec2[0];
		sum[1] = sum[1] + vec1[1] + vec2[1];
		vec1 = vectorDir(matrix[index - i]);
		vec2 = vectorDir(matrix[index + i]);
		sum[0] = sum[0] + vec1[0] + vec2[0];
		sum[1] = sum[1] + vec1[1] + vec2[1];
		for (int j = 1; j < 5 - i; j++) {
			vec1 = vectorDir(matrix[index - X * i + j]);
			vec2 = vectorDir(matrix[index - X * i - j]);
			sum[0] = sum[0] + vec1[0] + vec2[0];
			sum[1] = sum[1] + vec1[1] + vec2[1];
		}
	}
...
__device__ float* vectorDir(int X) {
	float vec[2] = { 0 };
	switch (X) {
	case 0:
		return vec;
...
cudaError_t allocateMemoryOnDevice(int * matrixA, int * matrixV) {
	...
	filterVector << <numBlocks, threadsPerBlock >> >(d_matrixA, d_matrixV, IMAGE_X + APRON * 2, IMAGE_Y + APRON * 2);
